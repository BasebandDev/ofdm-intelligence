...
GenerateOFDMChannel - Gives the frequency domain channel response for each of the OFDM symbol in each subcarrier, so it uses ResourceGrid's output
as one of the parameter, other being the channel model which has the channel impulse response parameters. So we get the channel impulse response for tire time-freq grid.
